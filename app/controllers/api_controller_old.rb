  ...
  def posts
    ...
    bm_result = Benchmark.measure do
      ...
      if params['tag']
        ...
        @posts_con = @posts_con.uniq { |p| p["id"]}
      else
      ...
      if direction == 'asc'
        ...
        when "reads"
          ...
          sorted_array_con = @posts_con.sort_by{ |e| e['reads'].to_i }
        when "likes"
          sorted_array = @posts.sort_by{ |e| e['likes'].to_i }
          sorted_array_con = @posts_con.sort_by{ |e| e['likes'].to_i }
        when "popularity"
          sorted_array = @posts.sort_by{ |e| e['popularity'].to_i }
          sorted_array_con = @posts_con.sort_by{ |e| e['popularity'].to_i }
        when "id"
          sorted_array = @posts.sort_by{ |e| e['id'].to_i }
          sorted_array_con = @posts_con.sort_by{ |e| e['id'].to_i }
        else
          sorted_array = @posts.sort_by{ |e| e['id'].to_i }
          sorted_array_con = @posts_con.sort_by{ |e| e['id'].to_i }
        end
        ...
        when "reads"
          ...
          sorted_array_con = @posts_con.sort_by{ |e| e['reads'].to_i }.reverse
        when "likes"
          sorted_array = @posts.sort_by{ |e| e['likes'].to_i }.reverse
          sorted_array_con = @posts_con.sort_by{ |e| e['likes'].to_i }.reverse
        when "popularity"
          sorted_array = @posts.sort_by{ |e| e['popularity'].to_i }.reverse
          sorted_array_con = @posts_con.sort_by{ |e| e['popularity'].to_i }.reverse
        when "id"
          sorted_array = @posts.sort_by{ |e| e['id'].to_i }.reverse
          sorted_array_con = @posts_con.sort_by{ |e| e['id'].to_i }.reverse
        else
          sorted_array = @posts.sort_by{ |e| e['id'].to_i }.reverse
          sorted_array_con = @posts_con.sort_by{ |e| e['id'].to_i }.reverse
        end
    ...
    @posts_con = {
      ...
      "posts_count": sorted_array_con.count,
      "posts": sorted_array_con
    }
